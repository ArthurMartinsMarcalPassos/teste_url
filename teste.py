...
def editar_url():
    ...
    if selecionado:
        ...
        nova_url = tk.simpledialog.askstring("Editar URL", "Digite a nova URL:", initialvalue=url_atual)  # Exibe uma janela de diálogo para editar a URL
        if nova_url:
            url_listbox.delete(selecionado)  # Remove a URL atual da ListBox
            url_listbox.insert(selecionado, nova_url)  # Insere a nova URL na mesma posição
...
def testar_url_selecionada():
    ...
    if selecionado:
        ...
        try:
            response = requests.get(url)  # Faz uma solicitação GET à URL usando a biblioteca requests
            if response.status_code == 200:
                messagebox.showinfo("URL Testada", f"A URL {url} está ativa!")  # Exibe uma mensagem informando que a URL está ativa
            else:
                messagebox.showwarning("URL Testada", f"A URL {url} retornou um código de status {response.status_code}!")  # Exibe uma mensagem informando o código de status retornado pela URL
        except requests.exceptions.RequestException:
            messagebox.showerror("URL Testada", f"A URL {url} não pôde ser acessada!")  # Exibe uma mensagem de erro caso não seja possível acessar a URL
...
adicionar_button = tk.Button(url_frame, text="Adicionar", command=adicionar_url)
...
testar_button = tk.Button(url_frame, text="Testar URL Selecionada", command=testar_url_selecionada)
